...
void Town_Guard::move_towards(){
	...
	//you are there
	if(((-32 < x_dif) && (x_dif < 32)) && ((-32 < y_dif) && (y_dif< 32))){
	...
	//go left and down
		...
		if(this->x_or_y){
			moved = move(E);
			if(moved){
				this->ret_path.insert(this->ret_path.end(),std::make_pair(1,E));
			}
		}else{
			moved = move(S);
			if(moved){
				this->ret_path.insert(this->ret_path.end(),std::make_pair(1,S));
			}
		}
		...
		if(this->x_or_y){
			...
			moved = move(N);
			if(moved){
				this->ret_path.insert(this->ret_path.end(),std::make_pair(1,N));
			}
		}
		...
		if(this->x_or_y){
			moved = move(W);
			if(moved){
				this->ret_path.insert(this->ret_path.end(),std::make_pair(1,W));
			}
		}else{
			moved = move(S);
			if(moved){
